    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            if form.is_valid():
                ...
                meep.save()
                messages.success(request,("your tweet has been successfull "))
...
def profile(request, pk):
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            # deciding to follow or unfollow
            if action == "unfollow":
        ...
        messages.success(request, ("you must be log in to acess this page"))
        return redirect('home')
...
def login_user(request):
    if request.method == "POST":
        username = request.POST['username']    # 'username' : this 'username' is taken from login.html page
        password = request.POST['password']    # 'password' : this 'password' is taken from login.html page
        user = authenticate(request, username=username, password=password )
        if user is not None:
            login(request, user)
            messages.success(request, ("you are logged in. happy Tweets"))
            return redirect('home')
        else:
...
def update_user(request):
    if request.user.is_authenticated:
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            messages.success(request, ("You must logged in to see this page"))
            return redirect('home')
